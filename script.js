const backendUrl = "https://mera-box-backend.onrender.com";  // ✅ Correct backend URL
...
function playVideo() {
  ...
  if (url.includes("youtube.com") || url.includes("youtu.be")) {
    ...
    alert("TeraBox preview not supported. You can only download.");

  } else if (url.includes("xvideos.com")) {
    iframe.style.display = "none";
    alert("XVideos preview not supported.");
...
async function detectCountryAndSetPrice() {
  try {
    ...
    document.getElementById("priceText").textContent =
      `Premium Price: ${price} (${data.country_name})`;
  } catch {
